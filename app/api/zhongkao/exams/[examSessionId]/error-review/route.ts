...
import {
  ...
  defaultExamErrorReviewServiceDeps,
  getExamErrorReview,
...
const ERROR_REVIEW_REQUEST_BODY_MAX_BYTES = 2 * 1024 * 1024;
...
async function readErrorReviewRequest(req: NextRequest): Promise<unknown> {
  ...
  for (;;) {
    ...
    if (total > ERROR_REVIEW_REQUEST_BODY_MAX_BYTES) {
...
export async function GET(req: NextRequest, { params }: Params): Promise<Response> {
  ...
  return withRequestOwnerId(req, async (ownerId, responseHeaders) => {
    ...
    try {
      ...
      const errorReview = await getExamErrorReview(
        await defaultExamErrorReviewServiceDeps(ownerId),
        examSessionId,
      );
      return NextResponse.json({ errorReview }, { status: 200, headers });
    } catch (error) {
...
export async function POST(req: NextRequest, { params }: Params): Promise<Response> {
  ...
  return withRequestOwnerId(req, async (ownerId, responseHeaders) => {
    ...
    try {
      const { examSessionId } = await params;
      if (!isExamSessionId(examSessionId)) return examNotFound(headers);
      const input = await readErrorReviewRequest(req);
      const result = await confirmExamErrorReview(
        await defaultExamErrorReviewServiceDeps(ownerId),
        examSessionId,
        input,
      );
      return NextResponse.json(
        { examSessionId: result.examSessionId, errorReview: result.errorReview },
